...
このfoundation-offcanvasブランチはCSSフレームワークのFoundationにOff Canvasプラグインを組み込んだバージョンです。一般的な用途やLaravelの学習にはoriginal-cssバージョンをご利用ください。コードがすっきりとして見やすくなっています。
...
jquery.offcanvas.jsは、モバイル表示時にメニュー項目による切り替えが動かないため、一部をコメントにしています。ページ切り替えをスクロールで行うための部分です。もし、お手元のモバイル機器で問題が無いようでしたら、コメントアウトしてください。
