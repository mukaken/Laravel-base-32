...
Foundationはレスポンシブ対応のCSSフレームワークです。

Twitter Bootstrapと同じような機能を提供してくれます。違いは、Bootstrapと比べ、デザインの変更が容易なことです。Bootstrapは出来上がっている部品を組み合わせて、お手軽にサイトを作成することに主眼を置いていますが、Foundationの場合、部品は荒削りですが変更をしやすく作っています。

デザインはfoundation.cssで決めていますが、このファイルを直接変更せず、app.cssに変更部分を書いてください。Foundationは頻繁にアップデートされます。直接変更するとアップデートが困難になります。app.cssはfoundation.cssより後に指定してあるため、app.cssの中に書いた記述のほうが有効になります。

サイトのデザインについては、多くのテンプレートが用意されています。views/foundationフォルダーの中に取り込んであります。本ブランチのデザインは、その中のsidebar.htmlを利用し、作成しました。

詳しくは、[Foundationの公式サイト](http://foundation.zurb.com/)をご覧ください。
...
この設定ファイルを直接変更してしまうと、バンドルをアップデートする際、上書きする可能性があります。そこで、設定を直接変更するのではなく、Laravelのイベントを利用し、変更する方法を紹介しています。
