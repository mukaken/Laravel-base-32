...
> **注意：**外部キーで参照されるフィールドは自動増分項目であり、そのため自動的にunsigned integerになります。ですから、外部キーのフィールドは**unsigned()**で作成し、両方共に同じタイプであることを確認してください。さらに、両方のテーブルはエンジンに**InnoDB**をセットしていること、参照されるテーブルは、外部キーのテーブルの**前に**作成することも確実に行なってください。
